...
$sql = "SELECT idInspector, nombre, nivelModalidad, telefono, correo  FROM Inspectores WHERE 1=1";
      ...
      <tbody>
        <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($row['nivelModalidad']) ?></td>
            <td><?= htmlspecialchars($row['telefono']) ?></td>
            <td><?= htmlspecialchars($row['correo']) ?></td>
            <td>
              <a href="editarInspectores.php?id=<?= $row['idInspector'] ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil-fill"></i></a>
              <a href="eliminarInspectores.php?id=<?= $row['idInspector'] ?>" class="btn btn-danger btn-sm"><i class="bi bi-trash-fill"></i></a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
