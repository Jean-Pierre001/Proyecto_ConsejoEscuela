...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateName() {
        ...
        if (name === '') {
            // Sin nombre, no bloquear botón, solo limpiar mensaje
            messageEl.textContent = '';
            submitBtn.disabled = false;
            return;
        }

        fetch('check_school_name.php?schoolName=' + encodeURIComponent(name))
            .then(res => res.json())
            .then(data => {
                if (data.exists) {
                    messageEl.textContent = '⚠ Ya existe una escuela con este nombre.';
            ...
            .catch(err => {
                console.error(err);
                // En caso de error, mejor permitir enviar para no bloquear sin razón
                submitBtn.disabled = false;
            });
    }

    // Validar en cada cambio del input
